...
fn handle_mobile_share_connection(
    ...
    let Some(route) = (method == "GET")
        .then(|| authorized_mobile_share_route(&path, "share", &token, true))
        .flatten()
    else {
        let _ = write_http_response(
            &stream,
            ...
            let _ = write_http_response(
                ...
                &[
                    ...
                    (
                        ...
                        "default-src 'none'; style-src 'unsafe-inline'; script-src 'unsafe-inline'; connect-src ws: wss:",
                    ),
                ],
            );
        }
        MobileShareRoute::Download => serve_mobile_share_download(&mut stream, &session.log_path),
        MobileShareRoute::Events => {
...
mod tests {
    ...
    use super::{
        ...
        ActiveMobileShare, ActiveMobileWorkspaceShare, ActiveSession, ApplicationSettings,
        CaptureQuota, FlowControlSetting, LogIndexRecord, ParitySetting, SavedLogFingerprint,
